...
@Composable
fun DeleteLikePostStatus(vm: PostViewModel = hiltViewModel()) {

    val response = vm.deleteResponse
    ...
    when (response) {
        ...
        is Response.Success -> {
            Toast.makeText(LocalContext.current, "Ya no me gusta", Toast.LENGTH_SHORT).show()
        }
        is Response.Failure -> {
